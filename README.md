...
- Để điều hướng traffic vào 2 services của 2 namespace khác nhau, ta có 2 giải pháp:
  - Mỗi namespace sẽ triển khai 1 Ingress riêng với domain riêng
  - Sử dụng 1 Ingress chung để điều hướng traffic qua 2 services trung gian, 
...
```shell script
  Dev Environment: 
    ...
    kubectl apply -f CronJob-EveryMinute.yaml --namespace dev-custom-ns
    
  Stg Environment:
    ./03.deploy-stg-env.sh
    ------------------------------------------------------
    kubectl apply -f 01.NS+Ingress.yaml
    kubectl apply -f CronJob-EveryMinute.yaml --namespace stg-custom-ns
```
...
```shell script
    ./04.delete-all.sh
    -------------------------------------------------------
    kubectl delete -f CronJob-EveryMinute.yaml
